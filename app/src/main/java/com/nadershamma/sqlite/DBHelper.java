package com.nadershamma.sqlite;
...
public class DBHelper extends SQLiteOpenHelper{
    ...
    public LinkedList<Data> Search(String enquiery) {
        LinkedList<Data> list = new LinkedList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(enquiery, null);
        if (cursor.moveToFirst())
            do{
                int id_n = cursor.getColumnIndex(COLUMN_NAME);
                int id_s = cursor.getColumnIndex(COLUMN_SURNAME);
    ...
    public long Insert1000(){
        ...
        try {
            for (int i = 0; i != 1000; i++){
                ...
                statement.bindString(2,  "Stolikov");
                statement.bindLong(3,  1990);
